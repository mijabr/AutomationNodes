...
namespace AutomationPlayground.Scenes
{
    public class MijabrScene : IScene
    {
        ...
        public void Run(ClientContext clientContext)
        {
            ...
            @(300);Text(A).set([position:absolute,left:50%,top:50%,color:black]).transition([left:{clientContext.FontScaling * 30}px,top:0px,color:white,duration:700]);
            @(400);Text(B).set([position:absolute,left:50%,top:50%,color:black]).transition([left:{clientContext.FontScaling * 42}px,top:0px,color:white,duration:600]);
            @(500);Text(R).set([position:absolute,left:50%,top:50%,color:black]).transition([left:{clientContext.FontScaling * 54}px,top:0px,color:white,duration:500]);";
